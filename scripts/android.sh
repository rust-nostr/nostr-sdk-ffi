...
cargo run -p nostr-sdk-ffi --bin uniffi-bindgen generate --library "${TARGET_DIR}/aarch64-linux-android/release/${CDYLIB}" --config "${UNIFFI_CONFIG_PATH}" --language kotlin --no-format -o "${FFI_KOTLIN_DIR}"
